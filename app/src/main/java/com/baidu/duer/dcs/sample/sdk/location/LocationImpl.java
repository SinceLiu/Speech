 ...
 */
package com.baidu.duer.dcs.sample.sdk.location;

import android.content.Context;
import android.text.TextUtils;

import com.baidu.duer.dcs.sample.sdk.location.data.Gps;
import com.baidu.duer.dcs.sample.sdk.location.util.PositionUtil;
...
public class LocationImpl implements ILocation {
            ...
            try {
                if (location != null) {
                    ...
                    if (location.getLocType() == BDLocation.TypeGpsLocation
                        ...
                        // 转换为wgs84
                        try {
                            Gps gps = PositionUtil.bd09ToGps84(location.getLatitude(), location.getLongitude());
                            location.setLongitude(gps.getLongtitude());
                            location.setLatitude(gps.getlatitude());
                            location.setCoorType("wgs84");
                        } catch (Exception e) {
                            e.printStackTrace();
                        }
    ...
    private void initDefaultLocationInfo(LocationInfo info) {
        info.longitude = 116.404;
        info.latitude = 39.915;
        info.city = "北京";
    }
    ...
    private void saveLocation(JSONObject localJson) {
        try {
            PreferenceUtil.put(context, SAVE_PREFILE, localJson.toString());
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
    ...
    private LocationInfo getSaveLocation() {
        try {
            LocationInfo locationInfo = new LocationInfo();
            String saveLocalString = (String) PreferenceUtil.get(context, SAVE_PREFILE, "");
